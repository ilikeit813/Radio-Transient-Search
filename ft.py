#Updates, independent from LSL
#Sugestion, split voltage raw data in to chunks so each processer can read the file parallelly
...
def main(args):
    ...
    fcl = 6000+7000
    fch = fcl + 3343 #60000
    #                    (4306, 4309) one offset = nChunks*nFramesAvg skiped
    for offset_i in range(0   ,    1):# one offset = nChunks*nFramesAvg skiped
        ...
        for i in xrange(nChunks):
            ...
            for j in xrange(framesWork):
                ...
                count[aStand] +=  1
            # Calculate the spectra for this block of data in unit of intensity
            masterSpectra[i,0,:] = ((numpy.fft.fftshift(numpy.abs(numpy.fft.fft2(data[:2,:]))[:,1:])[:,fcl:fch])**2.).mean(0)/LFFT/2.
            masterSpectra[i,1,:] = ((numpy.fft.fftshift(numpy.abs(numpy.fft.fft2(data[2:,:]))[:,1:])[:,fcl:fch])**2.).mean(0)/LFFT/2.
        # Save the results to the various master arrays
        outname = "%s_%i_fft_offset_%.9i_frames" % (getopt.getopt(args,':')[1][0], beam,offset)
        numpy.save('jamie'+outname,masterSpectra)
